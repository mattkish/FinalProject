...
def add_cat_data(cur, conn, cat_data):
    ...
    for d in cat_data:
        cur.execute(''' 
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ''', (get_origin_id(origins, d['origin']), d['name'], d['family_friendly'], d['shedding'], d['general_health'], d['playfulness'], d['children_friendly'], d['grooming'], d['intelligence'], d['other_pets_friendly'], d['min_weight'], d['max_weight'], d['min_life_expectancy'], d['max_life_expectancy'],))
